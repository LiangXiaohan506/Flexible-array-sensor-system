...
uint8 buff[8192];/*定义一个8KB的缓存*/
uint8 Voltage[8192];//电压值及位置信息缓存
...
//			len=getSn_TX_FSR(SOCK_TCPC); 								  	         /*定义len为已接收数据的长度*/
//			if(len>0)
//			{
//				recv(SOCK_TCPC,buff,len); 							   		         /*接收来自Server的数据*/
//				buff[0]=0x00;  											                 /*添加字符串结束符*/
//			  buff[1]=0x01;
//			  buff[2]=0x02;
//			  buff[3]=0x03;
			
				//printf("%s\r\n",buff);
				send(SOCK_TCPC,Voltage,len);								     	         /*向Server发送数据*/
//			}	
